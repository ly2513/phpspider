...
foreach ($array as $i) {
    ...
    // 助攻榜链接地址
    $url  = 'http://www.dongqiudi.com/data?competition=' . $i . '&type=assist_rank';
    ...
    foreach ($rankArr as $key => $value) {
        ...
        if (!$rankArr) {
            ...
            db::update('dqd_rank', ['id' => $rankArr['id'], 'help_num' => $value['num']], 'id=' . $rankArr['id']);
            log::add('射手榜ID : ' . $rankId . '更新成功');
            echo '射手榜ID : ' . $rankId . '更新成功' . PHP_EOL;
        }
